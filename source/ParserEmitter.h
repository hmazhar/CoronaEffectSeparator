...
class ParserEmitter {
 public:
  ...
  static void Parse(ChParticleEmitter& emitter,
    ...
    char* token;
    if (!mval.IsObject()) {
      throw(ChException("Invalid particle emitter object"));
    }

    token = (char*)"outlet_height";
    if (mval.HasMember(token)) {
      if (!mval[token].IsNumber()) {
        throw(ChException("Invalid number after '" + std::string(token) + "'"));
      }
      (ChSharedPtr<ChRandomParticlePositionRectangleOutlet>(emitter_positions))->OutletHeight() =
          mval[token].GetDouble();
    }
    token = (char*)"outlet_width";
    if (mval.HasMember(token)) {
      if (!mval[token].IsNumber()) {
        throw(ChException("Invalid number after '" + std::string(token) + "'"));
      }
      (ChSharedPtr<ChRandomParticlePositionRectangleOutlet>(emitter_positions))->OutletWidth() =
          mval[token].GetDouble();
    }
    token = (char*)"flow_control_mode";
    if (mval.HasMember(token)) {
      ...
      if (!enum_parsed) {
        throw(ChException("Invalid flow control type after '" + std::string(token) +
                          "', use FLOW_PARTICLESPERSECOND or FLOW_MASSPERSECOND"));
      }
    }
    token = (char*)"particles_per_second";
    if (mval.HasMember(token)) {
      if (!mval[token].IsNumber()) {
        throw(ChException("Invalid number after '" + std::string(token) + "'"));
      }
    ...
    token = (char*)"use_particle_reservoir";
    if (mval.HasMember(token)) {
    ...
    token = (char*)"particle_reservoir";
    if (mval.HasMember(token)) {
      if (!mval[token].IsInt()) {
        throw(ChException("Invalid integer after '" + std::string(token) + "'"));
      }
      emitter.ParticleReservoirAmount() = mval[token].GetInt();
    }
    token = (char*)"mass_reservoir";
    if (mval.HasMember(token)) {
      if (!mval[token].IsNumber()) {
        throw(ChException("Invalid number after '" + std::string(token) + "'"));
      }
