...
void RunTimeStep(ChSystemParallelDVI* mSys, const int frame) {
  ...
  if (frame % 150 == 0) {
    ...
    gen->createObjectsBox(utils::POISSON_DISK, dist, ChVector<>(-0.1, 0.0796399719369236 + 0.56, -0.0817847646755438),
                          ChVector<>(0.14, 0, 0.14), ChVector<>(0, 0, 0));
  }
...
int main(int argc, char* argv[]) {
  ...
  }
  {
    ...
    utils::AddBoxGeometry(Truss.get_ptr(), ChVector<>(.5, .6, .002),
                          ChVector<>(0.178496411, 0.42749398, 0.15 - 0.081784765), QUNIT);
    utils::AddBoxGeometry(Truss.get_ptr(), ChVector<>(.5, .6, .002),
                          ChVector<>(0.178496411, 0.42749398, -0.15 - 0.081784765), QUNIT);
    utils::AddBoxGeometry(Truss.get_ptr(), ChVector<>(.002, .6, .2), ChVector<>(-0.25, 0.42749398, -0.0817847646755438),
                          QUNIT);
    utils::AddBoxGeometry(Truss.get_ptr(), ChVector<>(.002, .6, .2), ChVector<>(.67, 0.42749398, -0.0817847646755438),
                          QUNIT);
  ...
  }
  {
    ...
    //    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.22121571, 0.013748141, 0.448934435),
    //    ChVector<>(0.034434623, -1.123553005, 0.030630547), ChQuaternion<>(0.707107, -0.0, -0.707107, -0.0));
    //    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.010173215, 0.250351436, 0.448934435),
    //    ChVector<>(0.034434661, -1.22725133, 0.241673042), ChQuaternion<>(0.707107, -0.0, -0.707107, -0.0));
    //    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.011042497, 0.250103825, 0.448934435),
    //    ChVector<>(0.034434585, -1.227003564, -0.179542666), ChQuaternion<>(0.0, -0.707107, 0.0, 0.707107));
    //
    //    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.013251174, 0.254046191, 0.2035), ChVector<>(-0.401248638,
    //    -1.230946291, 0.031500059), ChQuaternion<>(0.0, 0.0, 0.0, 1.0));
    //    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.029255479, 0.249805368, 0.2035), ChVector<>(-0.061741847,
    //    -1.226705282, 0.031499996), ChQuaternion<>(-0.0, 0.0, 0.0, 1.0));
    //    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.019763692, 0.250242141, 0.2035), ChVector<>(0.209277317,
    //    -1.2271419, 0.031499947), ChQuaternion<>(-0.0, 0.0, 0.0, 1.0));
    //    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.015414096, 0.250130195, 0.2035), ChVector<>(0.467954961,
    //    -1.227029814, 0.031499901), ChQuaternion<>(1.0, -0.0, 0.0, -0.0));

    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.22121571, 0.013748141, 0.448934435),
                          ChVector<>(0.034434623, -.7, 0.030630547), ChQuaternion<>(0.707107, -0.0, -0.707107, -0.0));

    utils::AddCylinderGeometry(Other.get_ptr(), 0.045869262, 0.16, ChVector<>(0, -0.517959057661396, 0),
                               ChQuaternion<>(0.5, -0.5, -0.5, 0.5));
    utils::AddCylinderGeometry(Other.get_ptr(), 0.008766574, 0.16,
                               ChVector<>(-0.381566155791125, -0.515717402690516, 0),
                               ChQuaternion<>(0.5, -0.5, -0.5, 0.5));
    utils::AddCylinderGeometry(Other.get_ptr(), 0.012493547, 0.16, ChVector<>(0.492612868005681, -0.513858743768626, 0),
                               ChQuaternion<>(0.5, -0.5, -0.5, 0.5));
    utils::AddCylinderGeometry(Other.get_ptr(), 0.024619437, 0.16, ChVector<>(0.244352690753348, -0.517959057661396, 0),
                               ChQuaternion<>(0.5, -0.5, -0.5, 0.5));
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.121733706251767, 0.00876657438055399, 0.165),
                          ChVector<>(-0.324444835447566, -0.623217402690516, 0),
                          ChQuaternion<>(0.515154, 0.0, 0.0, 0.857097));
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.121733706251767, 0.00901290521459452, 0.165),
                          ChVector<>(-0.0943376359362894, -0.62333298893951, 0),
                          ChQuaternion<>(0.515154, 0.0, 0.0, -0.857097));
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.102920917614647, 0.00880730074763987, 0.165),
                          ChVector<>(0.0461378510072952, -0.614333388045196, 0),
                          ChQuaternion<>(0.739227, 0.0, 0.0, -0.673456));
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.110302982718799, 0.009073557274052, 0.165),
                          ChVector<>(0.201202979597005, -0.620296344571127, 0),
                          ChQuaternion<>(0.611299, 0.0, 0.0, -0.791400));
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.111244909897975, 0.00991722943912151, 0.165),
                          ChVector<>(0.287088436539427, -0.62133744838765, 0),
                          ChQuaternion<>(0.792869, 0.0, 0.0, -0.609392));
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.111244909897976, 0.01, 0.165),
    ...
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.115826155371625, 0.0147645993377839, 0.447),
    ...
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.115826155371625, 0.0147645993377839, 0.447),
                          ChVector<>(0.0545, -0.624455203991318, -0.106974099367162),
                          ChQuaternion<>(0.585725, 0.396140, -0.585725, -0.396140));
    utils::AddBoxGeometry(Other.get_ptr(), ChVector<>(0.111244909897976, 0.018292686227323, 0.16500005),
...

  {
    ...
    utils::InitializeObject(Conveyer, mass, mat_conveyor, pos, rot, true, false, 8, 8);
    // Conveyer->SetInertiaXX(ChVector<>(0.029314394817887, 0.0796399719369236, 0.0514634926681141));
    utils::AddBoxGeometry(Conveyer.get_ptr(), ChVector<>(0.193724352473189, 0.005, 0.2),
                          ChVector<>(0.0785377577161679, 0.554163002941846, 0), QUNIT);
