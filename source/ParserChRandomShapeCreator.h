...
class ParserChRandomShapeGenerator {
 public:
  ...
  static ChSharedPtr<ChRandomShapeCreator> ParseCreate(rapidjson::Value& mval, ChSystem& msystem) {
    if (!mval.IsObject()) {
      throw(ChException("Invalid shape creator object."));
    }
    ...
    if (mval.HasMember(token)) {
      if (!mval[token].IsString()) {
        throw(ChException("Invalid string after '" + std::string(token) + "'"));
      }
      ...
      if (mtype == std::string("ChRandomShapeCreatorSpheres")) {
        ...
        if (mval.HasMember(token)) {
          if (!mval[token].IsObject()) {
            throw(ChException("Invalid distribution after '" + std::string(token) + "'"));
          }
      ...
      if (mtype == std::string("ChRandomShapeCreatorFromFamilies")) {
        ...
        ChSharedPtr<ChRandomShapeCreatorFromFamilies> mcreator(
            new ChRandomShapeCreatorFromFamilies);
        parsed_creator = mcreator;

        char* protoken = (char*)"probability_mode";
        if (mval.HasMember(protoken)) {
          if (!mval[protoken].IsString()) {
            throw(ChException("Invalid string after '" + std::string(protoken) + "'"));
          }
          char buffer[200];
          ...
          if (!enum_parsed) {
            throw(ChException("Invalid probability type after '" + std::string(protoken) +
                              "', use PARTICLE_PROBABILITY or MASS_PROBABILITY"));
          }
        }

        char* famtoken = (char*)"families";
        if (mval.HasMember(famtoken)) {
          if (!mval[famtoken].IsArray()) {
            throw(ChException("Invalid array after '" + std::string(token) + "'"));
          }
          for (rapidjson::SizeType i = 0; i < mval[famtoken].Size(); i++) {
            ...
            if (mval[famtoken][i].HasMember(token)) {
              if (!mval[famtoken][i][token].IsNumber()) {
                throw(ChException("Invalid number (range from 0 to 1) after '" +
                                  std::string(token) + "'"));
              }
              mprob = mval[famtoken][i][token].GetDouble();
            } else {
              throw(ChException(
                  "Missing 'probability' field in a family of ChRandomShapeCreatorFromFamilies"));
            }

            // Create the family using the parser, and add it to the generator using the providen
            // probability
            mcreator->AddFamily(
                ParserChRandomShapeGenerator::ParseCreate(mval[famtoken][i], msystem), mprob);
          }
        }
      }
      ...
      if (parsed_creator) {
        ...
        ElectricParticleProperty::enum_material_type parsed_mattype =
            ElectricParticleProperty::e_mat_other;
        ...
        if (mval.HasMember(token)) {
          ...
          if (mval[token].GetString() == std::string("plastic")) {
            parsed_mattype = ElectricParticleProperty::e_mat_plastic;
            parsed_conductivity = 0;
          }
          if (mval[token].GetString() == std::string("metal")) {
            parsed_mattype = ElectricParticleProperty::e_mat_metal;
            parsed_conductivity = 6428000;
          }
          if (mval[token].GetString() == std::string("other")) {
            parsed_mattype = ElectricParticleProperty::e_mat_other;
            parsed_conductivity = 0;
          }
          if (parsed_conductivity == -1) {
            throw(ChException(
                "Invalid material_type, only \"plastic\"|\"metal\"|\"other\" are supported."));
          }
        }
        ...
        token = (char*)"color";
        if (mval.HasMember(token)) {
          if (!mval[token].IsArray()) {
            throw(ChException("Invalid array after '" + std::string(token) + "'"));
          }
          if (mval[token].Size() != 3) {
            throw(
                ChException("Invalid color array size: must have 3 numbers (RGB, in 0..1 range)."));
          }
          if (!mval[token][rapidjson::SizeType(0)].IsNumber()) {
            throw(ChException("Invalid number for R color componenent (must be in 0..1 range)."));
          }
          parsed_color.R = (float)(mval[token][rapidjson::SizeType(0)].GetDouble());
          if (!mval[token][rapidjson::SizeType(1)].IsNumber()) {
            throw(ChException("Invalid number for G color componenent (must be in 0..1 range)."));
          }
          parsed_color.G = (float)(mval[token][rapidjson::SizeType(1)].GetDouble());
          if (!mval[token][rapidjson::SizeType(2)].IsNumber()) {
            throw(ChException("Invalid number for B color componenent (must be in 0..1 range)."));
          }
          ...
          class MyCreator_custom : public ChCallbackPostCreation {
           ...
           public:
            virtual void PostCreation(ChSharedPtr<ChBody> mbody,
              ...
              // in case the electric_asset is later processed for data output, maybe usefult to
              // initialize it too
    ...
    if (parsed_creator.IsNull()) {
      throw(ChException("Cannot create the specified particle creator (check after 'type': .. )"));
    }
