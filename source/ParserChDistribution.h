...
class ParserChDistribution {
 public:
  ...
  static ChSmartPtr<ChDistribution> ParseCreate(rapidjson::Value& mval) {
    if (!mval.IsObject()) {
      throw(ChException("Invalid distribution object."));
    }
    ...
    if (mval.HasMember(token)) {
      ...
      if (mtype == std::string("ChContinuumDistribution")) {
        ...
        if (mval.HasMember(token)) {
          if (!mval[token].IsArray()) {
            throw(ChException("Invalid x array in ChContinuumDistribution."));
          }
          for (rapidjson::SizeType i = 0; i < mval[token].Size(); i++) {
            if (!mval[token][i].IsNumber()) {
              throw(ChException("Invalid number in x array of ChContinuumDistribution." +
                                std::string(token) + "'"));
            }
        ...
        if (mval.HasMember(token)) {
          if (!mval[token].IsArray()) {
            throw(ChException("Invalid y array in ChContinuumDistribution."));
          }
          for (rapidjson::SizeType i = 0; i < mval[token].Size(); i++) {
            if (!mval[token][i].IsNumber()) {
              throw(ChException("Invalid number in y array of ChContinuumDistribution." +
                                std::string(token) + "'"));
            }
        ...
        if (mx.size() == 0) {
          throw(ChException("Error, empty x array in ChContinuumDistribution"));
        }
        if (my.size() == 0) {
          throw(ChException("Error, empty y array in ChContinuumDistribution"));
        }
        if (mx.size() != my.size()) {
          throw(ChException(
              "Error, x and y arrays must have same length, in ChContinuumDistribution. "));
        }
      ...
      if (mtype == std::string("ChDiscreteDistribution")) {
        ...
        if (mval.HasMember(token)) {
          if (!mval[token].IsArray()) {
            throw(ChException("Invalid x array in ChDiscreteDistribution."));
          }
          for (rapidjson::SizeType i = 0; i < mval[token].Size(); i++) {
            if (!mval[token][i].IsNumber()) {
              throw(ChException("Invalid number in x array of ChDiscreteDistribution."));
            }
            mx.push_back(mval[token][i].GetDouble());
          }
        }
        ...
        if (mval.HasMember(token)) {
          if (!mval[token].IsArray()) {
            throw(ChException("Invalid y array in ChDiscreteDistribution."));
          }
          for (rapidjson::SizeType i = 0; i < mval[token].Size(); i++) {
            if (!mval[token][i].IsNumber()) {
              throw(ChException("Invalid number in y array of ChDiscreteDistribution."));
            }
            my.push_back(mval[token][i].GetDouble());
          }
        }
        if (mx.size() == 0) {
          throw(ChException("Error, empty x array in ChDiscreteDistribution"));
        }
        if (my.size() == 0) {
          throw(ChException("Error, empty y array in ChDiscreteDistribution"));
        }
        if (mx.size() != my.size()) {
          throw(ChException(
              "Error, x and y arrays must have same length, in ChDiscreteDistribution. "));
        }
    ...
    if (createddistr.IsNull()) {
      throw(ChException("Cannot create the specified distribution (unknown 'type': ...? )"));
    }
