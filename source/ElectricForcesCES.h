  ...
  void apply_forces(ChSystem* msystem,          // contains all bodies
    ...
    for (unsigned int i = 0; i < msystem->Get_bodylist()->size(); i++) {
      ...
      if (was_a_particle) {
        ...
        // double rot_speedz=rot_speed.z; //bisogna tirare fuori la componente attorno all'asse z
        // globale della velocit� di rotazione

        double velocity_norm_sq = velocity.Length2();

        // ChQuaternion<> rot_velocity=abody->GetRot_dt;

        // Polar coordinates of particles respect to the axis of the rotor, may be useful later
        // **ALEX
        ...
        double average_rad = 0.5 * electricproperties->Cdim.Length();    // Approximate to sphere
                                                                         // radius. Ida: this can
        // be improved, by having Stokes forces for
        // three Cdim x y z values maybe
        ...
        double Ex = (((j - h1 + xuno) / (pow((j - h1 + xuno), 2) + pow(yuno, 2)) +
                      ((j + h1 - xuno) / (pow((j + h1 - xuno), 2) + pow(yuno, 2))) *
                          f));    // analytical expression of the electric field x direction***ida
        double Ey = ((yuno / (pow((j - h1 + xuno), 2) + pow(yuno, 2)) -
                      (yuno / (pow((j + h1 - xuno), 2) + pow(yuno, 2))) *
                          f));    // analytical expression of the electric field y direction***ida
        double Ez = 0;
        ...
        if (electricproperties->material_type == ElectricParticleProperty::e_mat_metal) {
          ...
          if ((distx > 0) && (disty > 0)) {
            if (electricproperties->chargeM == 0) {
              electricproperties->chargeM =
                  (2. / 3.) * pow(CH_C_PI, 3) * epsilon * pow(average_rad, 2) * E;
        ...
        if (electricproperties->material_type ==
            ElectricParticleProperty::e_mat_plastic)    // forze sulle particelle non metalliche
        {

          // charge the particle? (contact w. drum)
          if ((distx > 0.04) && (disty > 0)) {
            if (electricproperties->chargeP == 0) {
              electricproperties->chargeP = 3 * CH_C_PI * epsilonO * pow(2 * average_rad, 2) *
                                            Emax * (epsilonR / (epsilonR + 2));    // charge
          ...
          electricproperties->ElectricImageForce.x =
              -((pow(electricproperties->chargeP, 2)) /
                (4 * CH_C_PI * epsilon * pow((2 * average_rad), 2)) * cos(atan2(disty, distx)));
          electricproperties->ElectricImageForce.y =
              -((pow(electricproperties->chargeP, 2)) /
                (4 * CH_C_PI * epsilon * pow((2 * average_rad), 2)) * sin(atan2(disty, distx)));
